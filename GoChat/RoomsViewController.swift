...
class RoomsViewController: UIViewController {
    ...
    @IBAction func NewRoom(_ sender: Any) {
        if InputRoomName?.text != ""{                                   //房間名的input field不可為空
            ...
            //randomRoomNum = String(randomRoomNum)
            while(validRoomNum(roomNum: String(describing: randomRoomNum)) == false){                //如果檢查在資料庫不重複==false
                randomRoomNum = arc4random_uniform(9999)                //蓋掉舊的創一個新的四位數房號
            }
            
            let newRoomRef = roomRef.child(String(describing: randomRoomNum))       //定義firebase裡的reference
    ...
    private func validRoomNum(roomNum: String) -> Bool{
        var isValid = false
